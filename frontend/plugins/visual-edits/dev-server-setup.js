...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Note: Les origines Emergent ont été retirées pour rendre le projet autonome
      // Si vous avez besoin d'autoriser d'autres domaines, ajoutez-les ici
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@downpricer.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@downpricer.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
