...
export const AdminDashboardPage = () => {
  ...
  return (
    <AdminLayout>
      ...
      <div
        className="min-h-screen text-white p-4 sm:p-6 md:p-12 selection:bg-orange-500/30 bg-gradient-to-b from-[#090909] via-[#070707] to-black"
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 md:mb-12">
            ...
            <p className="mt-2 text-zinc-500 text-[10px] sm:text-sm font-medium uppercase tracking-wider italic">
              Vue rapide des métriques clés et des dernières demandes
            </p>
          </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <Card className="bg-[#0E0E0E] border-white/10 ring-1 ring-white/[0.03] rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl hover:ring-white/[0.06] transition-all">
                <CardHeader className="p-5 md:p-8 pb-4 border-b border-white/[0.06] flex flex-row items-center justify-between">
                  ...
                  {stats?.recent_demandes && stats.recent_demandes.length > 0 ? (
                    <div className="divide-y divide-white/[0.06]">
                      {stats.recent_demandes.map((demande) => (
                        <div
                          key={demande.id}
                          className="flex flex-col sm:flex-row sm:items-center justify-between p-5 md:p-6 hover:bg-white/[0.02] transition-all group gap-4"
                        >
                          <div className="flex items-center gap-4 md:gap-5 min-w-0">
                            <div className="h-10 w-10 shrink-0 rounded-xl bg-[#0B0B0B] border border-white/10 flex items-center justify-center text-zinc-600 group-hover:border-orange-500/30 group-hover:text-orange-500 transition-colors">
                              <FileText size={18} />
                            </div>
                            <div className="min-w-0">
                              <p className="text-sm font-bold text-white group-hover:text-orange-500 transition-colors truncate">
                                {demande.name}
                              </p>
                              <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600">
                                Budget: {demande.max_price}€
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-4">
                            <span
                              className={`text-[8px] md:text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-tighter border ${getStatusStyle(
                                demande.status
                              )}`}
                            >
                              {demande.status.replace(/_/g, ' ')}
                            </span>
                            <ArrowUpRight size={14} className="text-zinc-700 group-hover:text-white transition-colors hidden sm:block" />
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="py-16 sm:py-20 text-center">
                      <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em]">Zéro activité détectée</p>
                    </div>
                  )}
                </CardContent>
              </Card>
...
const KPICard = ({ label, value, icon, color }) => {
  ...
  return (
    <div className="bg-[#0E0E0E] border border-white/10 ring-1 ring-white/[0.03] p-5 md:p-6 rounded-2xl md:rounded-[1.5rem] hover:border-white/15 hover:ring-white/[0.06] transition-all group">
