...
from fastapi import (
    ...
    UploadFile, File, Form, Body, BackgroundTasks
)
...
# En production, Nginx sert directement /uploads/, pas besoin de monter ici
        ...
        await notify_admin(
            ...
            EventType.ADMIN_NEW_USER,
            {
                ...
                "details": (
                    ...
                    f'<tr><td>Nom</td><td>{user_data.first_name} {user_data.last_name}</td></tr>'
                    f'<tr><td>Email</td><td>{user_data.email}</td></tr>'
                    f'<tr><td>Date</td><td>{user_doc["created_at"]}</td></tr>'
                    f'</table></div>'
                ),
                "action_button": (
                    f'<p style="text-align: center;">'
                    f'<a href="{{{{ base_url }}}}/admin/users" class="button">Voir les utilisateurs</a>'
                    f'</p>'
                )
            },
            background_tasks
...
# ======================
# PUBLIC SETTINGS / CATEGORIES / ARTICLES
# ======================

@api_router.get("/settings/public")
async def get_public_settings():
    settings_docs = await db.settings.find(
        {"key": {"$in": ["logo_url", "contact_phone", "contact_email", "discord_invite_url", "billing_mode"]}},
        {"_id": 0}
    ).to_list(100)
    ...
    try:
        await notify_user(
            ...
            current_user.email,
            {
                ...
                "status_box": (
                    '<div class="success-box"><p style="margin: 0;">'
                    '<strong>✓ Votre demande a été enregistrée avec succès !</strong>'
                    "</p></div>"
                ),
                "message": (
                    f'Nous avons bien reçu votre demande "{demande_data.name}" '
                    "et nous allons l'étudier dans les plus brefs délais."
                ),
                ...
                "details": (
                    f"<table>"
                    f"<tr><td>ID Demande</td><td>{demande_id}</td></tr>"
                    f"<tr><td>Prix maximum</td><td>{demande_data.max_price}€</td></tr>"
                    f"<tr><td>Acompte</td><td>{demande_doc['deposit_amount']}€</td></tr>"
                    f"<tr><td>Statut</td><td>{demande_doc['status']}</td></tr>"
                    f"</table>"
                ),
                "action_button": (
                    f'<p style="text-align: center;">'
                    f'<a href="{{{{ base_url }}}}/demandes/{demande_id}" class="button">Voir ma demande</a>'
                    f"</p>"
                )
            },
            background_tasks
        ...
        await notify_admin(
            ...
            EventType.ADMIN_NEW_CLIENT_REQUEST,
            {
                ...
                "details": (
                    f'<div class="info-box"><table>'
                    f"<tr><td>ID Demande</td><td>{demande_id}</td></tr>"
                    f"<tr><td>Nom</td><td>{demande_data.name}</td></tr>"
                    f"<tr><td>Client</td><td>{user_doc.get('first_name','')} {user_doc.get('last_name','')} ({current_user.email})</td></tr>"
                    f"<tr><td>Prix maximum</td><td>{demande_data.max_price}€</td></tr>"
                    f"<tr><td>Prix de référence</td><td>{demande_data.reference_price}€</td></tr>"
                    f"<tr><td>Acompte</td><td>{demande_doc['deposit_amount']}€</td></tr>"
                    f"<tr><td>Statut</td><td>{demande_doc['status']}</td></tr>"
                    f"</table></div>"
                ),
                "action_button": (
                    f'<p style="text-align: center;">'
                    f'<a href="{{{{ base_url }}}}/admin/demandes/{demande_id}" class="button">Voir la demande</a>'
                    f"</p>"
                )
            },
            background_tasks
...
async def pay_deposit(demande_id: str, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        await db.demandes.update_one(
            {"id": demande_id},
            {"$set": {"status": DemandeStatus.DEPOSIT_PAID, "payment_type": payment_result.get("type", "UNKNOWN")}}
        )
...
async def cancel_demande(demande_id: str, current_user=Depends(get_current_user)):
    ...
    if demande["status"] in blocking_statuses and not is_admin:
        ...
        support_email = support_setting["value"] if support_setting else (
            contact_setting["value"] if contact_setting else "contact@downpricer.com"
        )
        raise HTTPException(
            status_code=400,
...
async def create_seller_sale(
    ...
    await db.articles.update_one({"id": sale_data.article_id}, {"$set": {"stock": new_stock}})
    ...
    try:
        await notify_admin(
            ...
            EventType.ADMIN_NEW_SALE,
            {
                ...
                "details": (
                    f'<div class="info-box"><table>'
                    f"<tr><td>Article</td><td>{article['name']}</td></tr>"
                    f"<tr><td>Prix de vente</td><td>{sale_data.sale_price}€</td></tr>"
                    f"<tr><td>Bénéfice</td><td>{profit}€</td></tr>"
                    f"<tr><td>Vendeur</td><td>{user_doc.get('first_name','')} {user_doc.get('last_name','')} ({current_user.email})</td></tr>"
...
# ======================
# ADMIN DASHBOARD / ARTICLES / CATEGORIES
# ======================
...
async def update_article(article_id: str, article_data: ArticleCreate):
    ...
    await db.articles.update_one({"id": article_id}, {"$set": article_data.model_dump()})
...
async def update_article_stock(article_id: str, data: dict):
    ...
    result = await db.articles.update_one({"id": article_id}, {"$set": {"stock": stock_val}})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Article non trouvé")

    return {"success": True, "message": f"Stock mis à jour: {stock_val}"}
...
async def update_article_visibility(article_id: str, visibility: dict = Body(...)):
    ...
    result = await db.articles.update_one({"id": article_id}, {"$set": update_fields})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Article non trouvé")
...
async def delete_article(article_id: str):
    result = await db.articles.update_one({"id": article_id}, {"$set": {"status": "deleted"}})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Article non trouvé")
...
# ======================
# ADMIN DEMANDES STATUS
# ======================

@api_router.put("/admin/demandes/{demande_id}/status", dependencies=[Depends(require_roles([UserRole.ADMIN]))])
async def admin_update_demande_status(demande_id: str, data: dict, background_tasks: BackgroundTasks):
    ...
    result = await db.demandes.update_one({"id": demande_id}, {"$set": update_data})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Demande non trouvée")
...
async def admin_get_all_settings():
    ...
    return {s["key"]: s["value"] for s in settings_docs}
...
async def admin_update_setting(key: str, data: dict):
    ...
    if existing:
        await db.settings.update_one({"key": key}, {"$set": {"value": value}})
    else:
...
async def test_email(background_tasks: BackgroundTasks, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        """
        text_body = (
            "Test de notification email - DownPricer\n\n"
...
# ======================
# ADMIN USERS / DEMANDES / SALES LISTING
# ======================
...
async def update_user_roles(user_id: str, roles: List[str]):
    result = await db.users.update_one({"id": user_id}, {"$set": {"roles": roles}})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Utilisateur non trouvé")
...
async def get_sale_detail(sale_id: str):
    ...
    return {"sale": sale, "article": article, "seller": seller}


@api_router.put("/admin/sales/{sale_id}/status", dependencies=[Depends(require_roles([UserRole.ADMIN]))])
async def update_sale_status(sale_id: str, payload: dict):
    new_status = payload.get("status")
    result = await db.seller_sales.update_one({"id": sale_id}, {"$set": {"status": new_status}})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Vente non trouvée")
            ...
            await notify_user(
                ...
                seller_email,
                {
                    ...
                    "details": (
                        ...
                        f"<tr><td>Bénéfice</td><td>{sale.get('profit',0)}€</td></tr>"
                        f"<tr><td>ID Vente</td><td>{sale_id}</td></tr>"
                        f"</table>"
                    ),
                    "action_button": (
                        f'<p style="text-align: center;">'
                        f'<a href="{{{{ base_url }}}}/seller/sales/{sale_id}" class="button">Effectuer le paiement</a>'
                        f"</p>"
                    )
                },
                background_tasks
...
async def reject_sale(sale_id: str, data: dict, background_tasks: BackgroundTasks):
    reason = data.get("reason", "")
...
async def confirm_payment(sale_id: str, background_tasks: BackgroundTasks):
    sale = await db.seller_sales.find_one({"id": sale_id}, {"_id": 0})
    if not sale:
        raise HTTPException(status_code=404, detail="Vente non trouvée")
    ...
    await db.seller_sales.update_one(
        {"id": sale_id},
        {"$set": {"status": SaleStatus.SHIPPING_PENDING, "updated_at": datetime.now(timezone.utc).isoformat()}}
    )
...
async def complete_sale(sale_id: str):
    await db.seller_sales.update_one(
        {"id": sale_id},
        {"$set": {"status": SaleStatus.COMPLETED, "updated_at": datetime.now(timezone.utc).isoformat()}}
    )
    return {"success": True, "message": "Vente terminée"}


# ======================
# SELLER PAYMENT SUBMISSION / DETAIL
# ======================
        ...
        await notify_admin(
            ...
            EventType.ADMIN_PAYMENT_PROOF_SUBMITTED,
            {
                ...
                "details": (
                    ...
                    f"<tr><td>Prix de vente</td><td>{sale.get('sale_price',0)}€</td></tr>"
                    f"<tr><td>Vendeur</td><td>{user_doc.get('first_name','')} {user_doc.get('last_name','')} ({current_user.email})</td></tr>"
                    f"<tr><td>Méthode</td><td>{data.get('method','')}</td></tr>"
                    f"<tr><td>ID Vente</td><td>{sale_id}</td></tr>"
                    f"</table></div>"
                ),
                "action_button": (
...
async def get_seller_sale_detail(sale_id: str, current_user=Depends(get_current_user)):
    ...
    article = await db.articles.find_one({"id": sale["article_id"]}, {"_id": 0})
    return {"sale": sale, "article": article}
    ...
    try:
        await notify_user(
            ...
            current_user.email,
            {
                ...
                "details": (
                    f"<table>"
                    f"<tr><td>Nom du site</td><td>{site_data.site_name}</td></tr>"
                    f"<tr><td>URL</td><td><a href='{{{{ base_url }}}}/{site_data.slug}'>{{{{ base_url }}}}/{site_data.slug}</a></td></tr>"
                    f"<tr><td>Plan</td><td>{site_data.plan_id}</td></tr>"
                    f"</table>"
                ),
        ...
        await notify_admin(
            ...
            EventType.ADMIN_NEW_MINISITE,
            {
                ...
                "details": (
                    f'<div class="info-box"><table>'
                    f"<tr><td>Nom du site</td><td>{site_data.site_name}</td></tr>"
                    f"<tr><td>Slug</td><td>{site_data.slug}</td></tr>"
                    f"<tr><td>Propriétaire</td><td>{current_user.email}</td></tr>"
                    f"<tr><td>Plan</td><td>{site_data.plan_id}</td></tr>"
                    f"<tr><td>ID Site</td><td>{site_id}</td></tr>"
                    f"</table></div>"
                ),
...
async def add_minisite_article(site_id: str, article_data: MiniSiteArticleCreate, current_user=Depends(get_current_user)):
    ...
    plan_quotas = {"SITE_PLAN_1": 5, "SITE_PLAN_10": 10, "SITE_PLAN_15": 20}
...
async def admin_update_minisite_status(site_id: str, data: dict = Body(...)):
    ...
    if status_val in ["suspended", "deleted"]:
        ...
        if minisite:
            ...
            if user:
                ...
                await db.users.update_one({"id": user["id"]}, {"$set": {"roles": new_roles}})

    result = await db.minisites.update_one({"id": site_id}, {"$set": update_data})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Mini-site non trouvé")

    return {"success": True, "message": f"Statut mis à jour: {status_val}"}
...
async def admin_get_all_minisite_articles():
    ...
    for article in articles:
        minisite = await db.minisites.find_one(
            ...
            {"_id": 0, "site_name": 1, "user_email": 1, "status": 1}
        )
        if minisite:
...
async def admin_update_minisite_article_status(article_id: str, data: dict = Body(...)):
    ...
    result = await db.minisite_articles.update_one({"id": article_id}, {"$set": update_data})
    if result.modified_count == 0:
        raise HTTPException(status_code=404, detail="Article non trouvé")

    return {"success": True, "message": f"Statut article mis à jour: {status_val}"}
